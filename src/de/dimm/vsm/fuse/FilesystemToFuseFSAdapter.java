...
public class FilesystemToFuseFSAdapter implements FuseFS
{
    ...
    public FilesystemToFuseFSAdapter( Filesystem3 fs3, Logger log )
    {
        this(fs3, System.getProperty("file.encoding", "UTF-8"), log);
    }

    public FilesystemToFuseFSAdapter( Filesystem3 fs3, String encoding, Logger log )
    {
        this(fs3, Charset.forName(encoding), log);
    }

    public FilesystemToFuseFSAdapter( Filesystem3 fs3, Charset cs, Logger log )
    {
        this.fs3 = fs3;
    ...
    public int readlink( ByteBuffer path, ByteBuffer link )
    {
        ...
        try
        {
            ...
            if (errno == 0)
            {
                ...
                CharsetEncoder enc = cs.newEncoder().onUnmappableCharacter(
                        CodingErrorAction.REPLACE).onMalformedInput(
                ...
                if (result.isOverflow())
                {
                    throw new FuseException(
                            "Buffer owerflow while encoding result").initErrno(Errno.ENAMETOOLONG);
                }
            }
            ...
            FuseSizeSetter sizeSetter )
    {
        ...
        try
        {
            return handleErrno(xattrSupport.getxattrsize(pathStr, nameStr,
    ...
    public int getxattr( ByteBuffer path, ByteBuffer name, ByteBuffer value )
    {
        ...
        try
        {
            return handleErrno(xattrSupport.getxattr(pathStr, nameStr, value),
                    value);
        }
        catch (Exception e)
        {
    ...
    private class XattrValueLister implements XattrLister
    {
        ...
        CharsetEncoder enc = cs.newEncoder().onMalformedInput(
