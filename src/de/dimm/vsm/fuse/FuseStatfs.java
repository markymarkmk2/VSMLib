...
public class FuseStatfs extends Struct implements FuseStatfsSetter
{
    ...
    public int blocksAvail = -1; // by default (if not overwriten) it is
    // computed as: blocksFree * 20 / 19
    public int files;
    ...
    protected boolean appendAttributes( StringBuilder buff, boolean isPrefixed )
    {
        ...
        buff.append("blockSize=").append(blockSize).append(", blocks=").append(
                blocks).append(", blocksFree=").append(blocksFree).append(
                ", blocksAvail=").append(blocksAvail).append(", files=").append(files).append(", filesFree=").append(filesFree).append(
                ", namelen=").append(namelen);
