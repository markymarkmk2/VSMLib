...
/**
 * This is a byte level filesystem API (in contrast to String level filesystem
 * API like fuse.Filesystem[123]). Any paths/names are passed as native
 * ByteBuffer objects or byte[] arrays. This is the interface that is called
 * from JNI bindings. It is not intended that this interface be implemented
 * directly by users but instead a fuse.Filesystem[123] interface should be
 * implemented and encoding of file names and paths should be left to a special
 * adapter class: fuse.Filesystem3ToFuseFSAdapter.
 * 
...
public interface FuseFS extends FilesystemConstants
{
    ...
    // <steven_pearson@final-step.com>
    // and then modified by Peter Levart <peter@select-tech.si> to fit the new
